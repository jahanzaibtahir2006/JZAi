...
useEffect(() => {
  ...
  if (!user?.id) {
    router.replace('/auth?redirect=/create-chatbot')
    return;
  }
...
if (editId) {
  ...
  fetch(`https://jzai-saas.jahanzaibtahir2006.workers.dev/bots?user_id=${user.id}`)
    .then((r) => r.json())
    .then((data) => {
      const bot = data.bots?.find((b: any) => String(b.id) === editId);
      if (bot) {
        setForm({
          botName: bot.name || "",
          industry: bot.industry || "",
          botLang: bot.language || "English",
          websiteUrl: bot.website_url || "",
          brandColor: bot.color || "#e8193c",
          businessDesc: bot.business_desc || "",
          faqs: bot.faqs || "",
          knowledgeUrl: "",
          greetMsg: bot.greeting_msg || "",
          tone: bot.tone || "Professional",
          role: bot.role || "Lead Capture",
          leadFields: bot.lead_fields ? JSON.parse(bot.lead_fields) : [],
          fallbackMsg: bot.fallback_msg || "",
          notifyEmail: bot.notify_email || "",
          fullName: "",
          company: "",
        });
      }
    ...
    .catch(() => {});
  return; // ← edit mode plan limit check skip
}
  // Normal mode — plan limit check
  fetch(`https://jzai-saas.jahanzaibtahir2006.workers.dev/bots?user_id=${user.id}`)
    .then((r) => r.json())
    .then((data) => {
      ...
      if (count >= limit) {
        ...
        setTimeout(() => {
          const el = document.getElementById("plan-limit-warning")
          if (el) {
...
const goStep = (n: number) => {
  setCurrentStep(n)
  setTimeout(() => {
    const el = document.getElementById("cc-form-top")
    if (el) {
      const y = el.getBoundingClientRect().top + window.scrollY - 130
      window.scrollTo({ top: y, behavior: 'smooth' })
    }
  ...
  const submitForm = async () => {
        if (!editBotId && !validateStep(4)) return;
        setDeploying(true);
        setDeployError(null);
        try {
          const userStr = localStorage.getItem("jzai_user");
...
const url = editBotId
  ? `https://jzai-saas.jahanzaibtahir2006.workers.dev/bots/${editBotId}`
  : `https://jzai-saas.jahanzaibtahir2006.workers.dev/bots`;

const res = await fetch(url, {
  method: editBotId ? "PATCH" : "POST",
  ...
  body: JSON.stringify({
    ...
    ...(editBotId ? {} : { plan: selectedPlan.name }),
  }),
});
...
if (editBotId) {
  ...
  setTimeout(() => router.push("/dashboard?tab=bots"), 1500);
}
  ...
  return (
    <>
      ...
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .cc-hero-bg{
          ...
          background:radial-gradient(ellipse 40% 40% at 60% 50%,rgba(232,25,60,0.07) 0%,transparent 65%);
        }
        .cc-grid-overlay{
          position:absolute;inset:0;
        ...
        @media(max-width:1000px){
          .cc-hero-bg{background:radial-gradient(ellipse 60% 40% at 50% 30%,rgba(232,25,60,0.08) 0%,transparent 70%);}
      ...
      <section className="cc-hero">
        ...
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "var(--red)", filter: "blur(140px)", opacity: 0.03, top: "-10%", right: "-5%", pointerEvents: "none" }} />        <div className="cc-hero-content">
...
{planLimitHit && (
  <div id="plan-limit-warning" style={{
    maxWidth: 700, margin: "0 auto 32px", padding: "0 60px",
  }}>
    <div style={{
      background: "var(--red-dim)", border: "1px solid rgba(232,25,60,0.3)",
      ...
      <div>
        ...
        <div style={{ fontSize: 14, color: "var(--text2)" }}>
          Your free trial allows only 1 chatbot. Upgrade to Starter or Pro to create more.
        </div>
      </div>
        ...
        <>
          ...
          <div className="cc-steps-bar">
            <div className="cc-steps-row">
              {STEPS.filter(s => editBotId ? s.num <= 3 : true).map((s) => (
                <div
                  key={s.num}
          ...
          {/* ── MAIN WRAP ── */}
          <div id="cc-form-top" className="cc-main">
            {/* FORM CARD */}
              ...
              {currentStep === 3 && (
                <div>
                  ...
                  <div className="cc-form-footer">
                    ...
                    <button className="cc-btn-next" onClick={() => editBotId ? submitForm() : handleNextStep(4)}>
                      {editBotId ? "Save Changes 💾" : <>Continue <span className="cc-btn-arrow">→</span></>}
                    </button>
                  </div>
                </div>
              )}
      ...
      {success && (
        <div className="cc-success">
          ...
          <div className="cc-success-desc">
            Your bot is live! Copy your embed code from the dashboard and paste it on your website.
            <br /><br />
            <a href="/dashboard?tab=bots" style={{
              display: "inline-flex", alignItems: "center", gap: 8,
              background: "var(--red-dim)", border: "1px solid rgba(232,25,60,0.3)",
              color: "var(--red)", borderRadius: 8, padding: "10px 20px",
          ...
          <div className="cc-success-details">
            {[
              ...
              { key: "Status", val: "✓ Ready to embed", cls: "" },
              { key: "Active", val: "✓ Live", cls: "", style: { color: "#22c55e" } },
            ].map((r) => (
          ...
          <div className="cc-success-btns">
            <a href="/dashboard?tab=bots" className="cc-btn-primary-l">View Your Bots & Embed Code →</a>
