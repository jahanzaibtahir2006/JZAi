...
// ─── API base URL — ek jagah rakho, future mein change karna asaan ho ────────
const API = "https://jzai-saas.jahanzaibtahir2006.workers.dev";

export default function AuthPage() {
  // ── Theme & UI state ────────────────────────────────────────────────────────
  // FIX: "dark" default SSR safe hai — body script se flash nahi aata
  ...
  const [forgotLoading, setForgotLoading]   = useState(false); // FIX: loading state add kiya
  ...
  // FIX: localStorage se theme read karo sirf client side pe (SSR safe)
  useEffect(() => {
    const saved = (localStorage.getItem("theme") as "dark" | "light") || "dark";
    setTheme(saved);
  }, []);

  // Theme change hone pe DOM aur localStorage dono update karo
  useEffect(() => {
    document.documentElement.setAttribute("data-theme", theme);
    localStorage.setItem("theme", theme);
  }, [theme]);

  // Doosre tabs/windows se theme sync karo
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === "theme") setTheme((e.newValue as "dark" | "light") || "dark");
    };
  ...
  // FIX: field errors clear karo, alert sirf tab switch pe clear ho — submit ke beech nahi
  const clearFieldErrors = () => {
    setLoginEmailErr(""); setLoginPassErr("");
    setSignupEmailErr(""); setSignupPassErr(""); setSignupConfirmErr("");
  };

  // Tab switch pe saari errors + alert saaf karo
  const clearAll = () => { clearFieldErrors(); setAlert({ type: "", msg: "" }); };
  ...
  // FIX: try/catch add kiya — network error pe app crash nahi karega
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // User ko localStorage mein save karo, phir dashboard pe bhejo
        localStorage.setItem("jzai_user", JSON.stringify(data.user));
        showAlert("success", "Signed in! Redirecting…");
        setTimeout(() => { window.location.href = "/dashboard"; }, 1200);
      }
    } catch {
      // Network failure ya CORS error
      showAlert("error", "Network error. Please check your connection.");
    } finally {
      setLoginLoading(false); // loading hamesha band karo, chahe error ho ya success
    }
  };

  // FIX: try/catch + finally add kiya
  const handleSignup = async () => {
    ...
    if (signupPass.length < 8)          { setSignupPassErr("Password must be at least 8 chars.");   valid = false; }
    ...
    try {
      const res  = await fetch(`${API}/auth/signup`, {
        ...
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({
          name:     `${signupFirst.trim()} ${signupLast.trim()}`,
          email:    signupEmail,
          password: signupPass,
        }),
      });
  ...
  // FIX: Real API call add kiya — pehle sirf fake alert tha
  const handleForgot = async () => {
    ...
    try {
      ...
      // 404 ya backend error hone par bhi same message dikhao (security best practice —
      // attacker ko pata na chale ke email exist karta hai ya nahi)
      if (!res.ok && res.status !== 404) {
        showAlert("error", data.error || "Something went wrong. Try again.");
      } else {
        showAlert("success", "If this email exists, a reset link has been sent.");
        setForgotEmail("");
      }
    } catch {
      showAlert("error", "Network error. Please try again.");
  ...
  // Enter key se form submit — tab ke hisaab se sahi handler call karo
  ...
  return (
    <>
      ...
      {/* ── Global styles (CSS variables + components) ── */}
      <style jsx global>{`
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        [data-theme="dark"] {
          ...
          --grid-line:rgba(232,25,60,0.04); --shadow:rgba(0,0,0,0.4); --footer-bg:#13131a;
        }
        [data-theme="light"] {
          ...
          --grid-line:rgba(208,16,46,0.05); --shadow:rgba(0,0,0,0.08); --footer-bg:#f0f0f5;
        }

        body { font-family:'DM Sans',sans-serif; background:var(--bg); color:var(--text); overflow-x:hidden; min-height:100vh; transition:background var(--transition-theme),color var(--transition-theme); }

        nav { position:fixed; top:0; left:0; right:0; z-index:100; display:flex; align-items:center; justify-content:space-between; padding:22px 60px; background:var(--nav-bg); backdrop-filter:blur(20px); border-bottom:1px solid var(--border2); transition:padding 0.3s,background var(--transition-theme); }
        ...
        .theme-toggle { position:relative; width:52px; height:28px; background:var(--toggle-bg); border:1.5px solid var(--toggle-border); border-radius:14px; cursor:pointer; outline:none; display:flex; align-items:center; padding:0 3px; transition:background var(--transition-theme); }
        .toggle-thumb { width:20px; height:20px; border-radius:50%; background:var(--red); display:flex; align-items:center; justify-content:center; font-size:11px; transition:transform 0.35s cubic-bezier(0.34,1.56,0.64,1); box-shadow:0 2px 6px var(--shadow); user-select:none; }
        [data-theme="light"] .toggle-thumb { transform:translateX(24px); }

        .page { min-height:100vh; display:flex; align-items:center; justify-content:center; padding:100px 24px 60px; position:relative; overflow:hidden; }
        .hero-bg { position:absolute; inset:0; background:radial-gradient(ellipse 60% 60% at 50% 40%,var(--red-glow),transparent 65%),radial-gradient(ellipse 30% 30% at 10% 90%,rgba(232,25,60,0.04),transparent 60%); }
        .grid-overlay { position:absolute; inset:0; background-image:linear-gradient(var(--grid-line) 1px,transparent 1px),linear-gradient(90deg,var(--grid-line) 1px,transparent 1px); background-size:60px 60px; animation:gridShift 20s linear infinite; }
        @keyframes gridShift { 0%{background-position:0 0} 100%{background-position:60px 60px} }

        .auth-wrap { position:relative; z-index:2; display:grid; grid-template-columns:1fr 1fr; max-width:960px; width:100%; background:var(--card-bg); border:1px solid var(--border); border-radius:20px; overflow:hidden; box-shadow:0 40px 80px var(--shadow); animation:fadeUp 0.7s ease 0.1s both; }
        @keyframes fadeUp { from{opacity:0;transform:translateY(24px)} to{opacity:1;transform:translateY(0)} }

        .left-panel { background:var(--bg2); padding:52px 48px; border-right:1px solid var(--border); display:flex; flex-direction:column; justify-content:space-between; position:relative; overflow:hidden; }
        .left-panel::before { content:''; position:absolute; top:-80px; right:-80px; width:260px; height:260px; border-radius:50%; background:radial-gradient(circle,var(--red-glow),transparent 70%); pointer-events:none; }
        ...
        input { width:100%; background:var(--bg2); border:1px solid var(--border); border-radius:8px; padding:13px 16px 13px 40px; font-size:14px; font-family:'DM Sans',sans-serif; color:var(--text); outline:none; transition:border-color 0.2s,box-shadow 0.2s,background var(--transition-theme); }
        ...
        .btn-submit { width:100%; background:var(--red); color:#fff; border:none; border-radius:8px; padding:14px; font-size:15px; font-weight:600; cursor:pointer; font-family:'DM Sans',sans-serif; display:flex; align-items:center; justify-content:center; gap:10px; transition:background 0.2s,transform 0.2s,box-shadow 0.2s; margin-top:6px; }
        .btn-submit:hover:not(:disabled) { background:var(--red-dark); transform:translateY(-1px); box-shadow:0 8px 28px var(--red-glow); }
          ...
          <div className="right-panel">
            ...
            <div className="tab-row">
              <button className={`tab-btn${tab === "login" ? " active" : ""}`}
                onClick={() => { setTab("login"); clearAll(); }}>Sign In</button>
              <button className={`tab-btn${tab === "signup" ? " active" : ""}`}
                onClick={() => { setTab("signup"); clearAll(); }}>Create Account</button>
            </div>

            {/* Global alert box — error ya success */}
            {alert.type && (
            ...
            {tab === "login" && (
              <>
                ...
                <div className="terms-text">
                  ...
                  <a onClick={() => { setTab("signup"); clearAll(); }}>Create one free →</a>
                </div>
              </>
            )}
            ...
            {tab === "signup" && (
              <>
                ...
                <div className="field-group">
                  ...
                  <div className="input-wrap">
                    <span className="input-ico">🔒</span>
                    <input type={showSignupPass ? "text" : "password"} placeholder="Min. 8 characters"
                      value={signupPass}
                      onChange={(e) => { setSignupPass(e.target.value); checkStrength(e.target.value); setSignupPassErr(""); }} />
                    <button className="eye-btn" type="button"
                ...
                <div className="field-group">
                  ...
                  <div className="input-wrap">
                    ...
                    <input type={showConfirmPass ? "text" : "password"} placeholder="Re-enter password"
                      value={signupConfirm}
                      onChange={(e) => { setSignupConfirm(e.target.value); setSignupConfirmErr(""); }} />
                    <button className="eye-btn" type="button"
            ...
            {tab === "forgot" && (
              <>
                ...
                <button className="btn-submit" onClick={handleForgot} disabled={forgotLoading}>
                  {forgotLoading ? <><span className="spinner" />Sending…</> : <>Send Reset Link <span>→</span></>}
                </button>

                <div className="terms-text" style={{ marginTop: "16px" }}>
                  <a onClick={() => { setTab("login"); clearAll(); }}>← Back to Sign In</a>
                </div>
              </>
            )}
...
// ─── Google SVG icon — alag component rakha taake JSX clean rahe ─────────────
function GoogleIcon() {
  return (
